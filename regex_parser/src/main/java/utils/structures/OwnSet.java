...
public class OwnSet<T> extends HashTable<T, T> implements Iterable<T> {
     ...
     *
     * <p>
     * Simpler and more intuitive method than having to call put directly</p>
     *
     * @param element
     */
    ...
    /**
     * Used to retrieve one element from the set
     * 
     * <p>In practice returns thee first element. If there are no elements, 
     * returns null.</p>
     * @return 
     */
    public T any() {
        ...
        int i = 0;
        OwnLinkedList<T,T> list = table[i]; 
        while(list == null || list.isEmpty()){
            i++; 
            list = table[i];
        }     
        return list.getFirstNode().getKey();
    }
     ...
     *
     * <p>
     * Does not modify the parameter set. Works exactly as putAll of HashTable,
     * only the name is different</p>
     *
     ...
     *
     * <p>
     * The iterator is called implicitly in for each loops, which is the main
     * reason behind implementing this method.</p>
     *
    ...
    /**
     * 
     * @return An OwnSet with identical contents but different address
     */
